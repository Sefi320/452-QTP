...
compute_pair_feats_monthEnd <- function(pairs, month_end, wide_prices){
  ...
  window_data <- wide_prices %>%
    dplyr::filter(date <= month_end & date >= valid_from) %>%
    dplyr::select(date, p1 = dplyr::all_of(leg1), p2 = dplyr::all_of(leg2)) %>%
    tidyr::drop_na() %>%
    dplyr::slice_tail(n = window) # this pulls the last n days of data from month end depending on the window used
  
  
  beta_ols <- coef(lm(p1 ~ p2, data = window_data))[2]

   spread <- window_data$p1 - beta_ols * window_data$p2 # this is the scaled spread that determines trade size
...
features_list <- foreach::foreach(
  ...
  .export = c("pair_grid", "month_ends", "dat_wide", "window",
