...
compute_pair_feats_monthEnd <- function(pairs, month_end, wide_prices){
  ...
  window_roll_data <- wide_prices %>%
    ...
    tidyr::drop_na() %>%
    dplyr::slice_tail(n = window_roll) # this pulls the last n days of data from month end depending on the window_roll used
  
  
  beta_ols <- coef(lm(p1 ~ p2, data = window_roll_data))[2]

   spread <- window_roll_data$p1 - beta_ols * window_roll_data$p2 # this is the scaled spread that determines trade size
